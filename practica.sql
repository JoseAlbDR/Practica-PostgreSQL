...
create table flota_keepcoding.modelo_vehiculo(
	id serial primary key,
	nombre varchar(50) not null,
	id_marca smallint not null
);

create table flota_keepcoding.marca_vehiculo( 
	id serial primary key,
...
create table flota_keepcoding.color_vehiculo( 
	id serial primary key,
	nombre varchar(30) not null
);
...
create table flota_keepcoding.poliza_seguro(
...
alter table flota_keepcoding.vehiculo add constraint pk_modelo foreign key (id_modelo) references flota_keepcoding.modelo_vehiculo(id);
alter table flota_keepcoding.vehiculo add constraint pk_color foreign key (id_color) references flota_keepcoding.color_vehiculo(id);
alter table flota_keepcoding.modelo_vehiculo add constraint pk_marca foreign key (id_marca) references flota_keepcoding.marca_vehiculo(id);
alter table flota_keepcoding.marca_vehiculo add constraint pk_grupo foreign key (id_grupo_empresarial) references flota_keepcoding.grupo_empresarial(id);
alter table flota_keepcoding.revision add constraint pk_moneda foreign key (id_moneda) references flota_keepcoding.moneda(id);
alter table flota_keepcoding.revision add constraint pk_vehiclo foreign key (matricula_vehiculo) references flota_keepcoding.vehiculo(matricula);
alter table flota_keepcoding.poliza_seguro add constraint pk_aseguradora foreign key (id_aseguradora) references flota_keepcoding.aseguradora(id);
alter table flota_keepcoding.poliza_seguro add constraint pk_vehiculo foreign key (matricula_vehiculo) references flota_keepcoding.vehiculo(matricula);
...
insert
	into
	flota_keepcoding.color_vehiculo(nombre) 
	select
	color
from
	flota_keepcoding.coches c
group by
	color;

insert
	into
	flota_keepcoding.grupo_empresarial(nombre) 
	select
	grupo
from
	flota_keepcoding.coches
group by
	grupo;

insert
	into
	flota_keepcoding.marca_vehiculo (nombre,
...
from
	flota_keepcoding.coches c
inner join flota_keepcoding.grupo_empresarial ge 
	on
	c.grupo = ge.nombre
group by
	ge.id,
...
insert
	into
	flota_keepcoding.modelo_vehiculo (nombre,
	id_marca)
	select
	c.modelo,
	mv.id
from
	flota_keepcoding.coches c
inner join flota_keepcoding.marca_vehiculo mv 
	on
	c.marca = mv.nombre
group by
	mv.id,
...
insert
	into
	flota_keepcoding.aseguradora(nombre) 
	select
	aseguradora
from
	flota_keepcoding.coches
...
insert
	into
	flota_keepcoding.vehiculo (matricula,
	km_total,
	fecha_compra,
	id_modelo,
	id_color)
select
	c.matricula,
	c.kms_totales,
	cast(c.fecha_compra as date),
	mv.id as id_modelo,
	cv.id as id_color
from
	flota_keepcoding.coches c
inner join flota_keepcoding.modelo_vehiculo mv on
	c.modelo = mv.nombre
inner join flota_keepcoding.color_vehiculo cv on
	c.color = cv.nombre
group by
	c.matricula,
	c.kms_totales,
...
insert
	into
	flota_keepcoding.poliza_seguro(fecha_poliza,
	num_poliza,
	id_aseguradora, matricula_vehiculo)
	select
	cast(c.fecha_alta_seguro as date),
	c.n_poliza,
	a.id,
	v.matricula
from
	flota_keepcoding.coches c
inner join flota_keepcoding.aseguradora a 
	on
	a.nombre = c.aseguradora
inner join flota_keepcoding.vehiculo v
	on
...
insert
	into
	flota_keepcoding.moneda(nombre) 
	select
	moneda
from
	flota_keepcoding.coches c
...
insert
	into
	flota_keepcoding.revision (km_revision,
	fecha_revision,
	importe,
	id_moneda,
	matricula_vehiculo)
select
	c.kms_revision,
	cast(c.fecha_revision as date),
	c.importe_revision,
	m.id as id_moneda,
	c.matricula
from
	flota_keepcoding.coches c
inner join flota_keepcoding.vehiculo v on
	v.matricula = c.matricula
inner join flota_keepcoding.moneda m on
	m.nombre = c.moneda
group by
	c.kms_revision,
	c.fecha_revision,
...
-- Borrar tabla temporal de datos --
drop table flota_keepcoding.coches;

-- CONSULTA --

-- Con todas las polizas --
select
	mv.nombre as modelo,
	mv2.nombre as marca,
	...
	ps.fecha_poliza
from
	flota_keepcoding.vehiculo v
inner join flota_keepcoding.modelo_vehiculo mv on
	v.id_modelo = mv.id
inner join flota_keepcoding.marca_vehiculo mv2 on
	mv.id_marca = mv2.id
inner join flota_keepcoding.grupo_empresarial ge on
	mv2.id_grupo_empresarial = ge.id
inner join flota_keepcoding.color_vehiculo cv on
	v.id_color = cv.id
inner join flota_keepcoding.poliza_seguro ps on
	v.matricula = ps.matricula_vehiculo
inner join flota_keepcoding.aseguradora a on
	ps.id_aseguradora = a.id
group by
	modelo,
	marca,
	grupo_empresarial,
...
-- Con la poliza con la fecha más reciente --
select
	mv.nombre as modelo,
	mv2.nombre as marca,
	ge.nombre as grupo_empresarial,
	...
	a.nombre as aseguradora,
	ps.num_poliza,
	ps.fecha_poliza as fecha_poliza_mas_reciente
from
	flota_keepcoding.vehiculo v
inner join flota_keepcoding.modelo_vehiculo mv on
	v.id_modelo = mv.id
...
inner join (
	select
		v.matricula,
		max(ps.fecha_poliza) as fecha_poliza
	from
		flota_keepcoding.vehiculo v
	inner join flota_keepcoding.poliza_seguro ps on
		v.matricula = ps.matricula_vehiculo
	...
	v.matricula = subquery.matricula
	and ps.fecha_poliza = subquery.fecha_poliza
inner join flota_keepcoding.aseguradora a on
	ps.id_aseguradora = a.id
group by
	modelo,
